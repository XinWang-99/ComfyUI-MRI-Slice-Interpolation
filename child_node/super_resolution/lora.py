...
def inject_trainable_lora(
    ...
    for _module, name, _child_module in _find_modules(
        ...
        require_grad_params.append({"params":_module._modules[name].lora_up.parameters()})
        require_grad_params.append({"params":_module._modules[name].lora_down.parameters()})
...
def inject_trainable_lora_extended(
    ...
    for _module, name, _child_module in _find_modules(
        model, target_replace_module, search_class, exclude_children_of
    ):
        ...
        if loras != None:
            _module._modules[name].lora_up.weight = nn.parameter.Parameter(loras.pop(0).to(torch.float32))
            _module._modules[name].lora_down.weight = nn.parameter.Parameter(loras.pop(0).to(torch.float32))
...
if __name__ == '__main__':
    ...
    unet_lora_params, train_names=inject_trainable_lora_extended(model,target_replace_module= ['ResnetBlockBigGANppSPADE'],search_class=[nn.Conv2d], exclude_children_of=[MySPADE],loras='./lora.pt')
    # save_lora_weight(model,path="./lora.pt",target_replace_module=['ResnetBlockBigGANppSPADE'])
    # print(22,model)


    # unet.requires_grad_(False)
    # unet_lora_params, train_names = inject_trainable_lora(unet)  
    # print(unet_lora_params.keys())